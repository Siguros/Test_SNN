...
# Attetention_spike
        ...
        c2_mem = c2_spike = torch.zeros(batch_size, self.scale, 32, 32, device=self.device)
    
        srb1_1_mem_1 = srb1_1_spike_1 = torch.zeros(batch_size, self.scale, 32, 32, device=self.device)
        srb1_2_mem_1 = srb1_2_spike_1 = torch.zeros(batch_size, self.scale, 32, 32, device=self.device)
        srb1_1_mem_2 = srb1_1_spike_2 = torch.zeros(batch_size, self.scale, 32, 32, device=self.device)
        srb1_2_mem_2 = srb1_2_spike_2 = torch.zeros(batch_size, self.scale*2, 32, 32, device=self.device)
...
# attention spike
        atten_srb2_2_mem, atten_srb2_2_spike = self.LIF2_2(attention_srb2_2_mem, attention_srb2_2_spike, decay_vector[4], self.SRB2_bn2(self.SRB2_conv2(srb2_2_spike))
