...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, use_conv=False):
        ...
        # Use 1*1 convolution to match the dimension if needed
    ...
    def from_digital(cls, digital_module, rpu_config, tile_module_class=None):
        # Generate an analog version of BasicBlock based on the settings of a digital module
        return cls(
...
class ResNetAttention1(nn.Module):
    def __init__(self, block, num_classes=100):  # num_classes 매개변수 추가
        ...
        self.fc = nn.Linear(1024 * block.expansion, num_classes)  # 출력 차원을 num_classes로 설정
...
class ResNetInput(nn.Module):
    def __init__(self, in_channels=3, base_channels=128, norm_layer=None):
...
class ResNetFeatures(nn.Module):
    def __init__(self, block, layers, zero_init_residual=False, groups=1, 
                 width_per_group=64, replace_stride_with_dilation=None, norm_layer=None, base_channels=128):
...
def create_input_module(in_channels=3, base_channels=128, norm_layer=None):
...
class IntegratedResNet(nn.Module):
    def __init__(self, architecture="resnet10", num_classes=10, rpu_config=None):
        ...
        in_features = 1024 * block_type.expansion  # 이 값은 실제 출력 특성 맵의 크기에 따라 달라질 수 있습니다.
