...
class Spiking_ResNet18_LIF_STBP(nn.Module):
    def __init__(
        ...
        self.SRB1_bn2 = nn.BatchNorm2d(self.scale*4)
        ...
        self.SRB2_bn2 = nn.BatchNorm2d(self.scale*4)
        ...
        self.SRB3_bn2 = nn.BatchNorm2d(self.scale*2*4)
        ...
        self.SRB4_bn2 = nn.BatchNorm2d(self.scale*2*4)
        ...
        self.SRB5_bn2 = nn.BatchNorm2d(self.scale*4*4)
        ...
        self.SRB6_bn2 = nn.BatchNorm2d(self.scale*4*4)
        ...
        self.SRB7_bn2 = nn.BatchNorm2d(self.scale*8*4)
        ...
        self.SRB8_bn2 = nn.BatchNorm2d(self.scale*8*4)
        ...
        self.tau_vector = torch.ones(18) * init_tau
    ...
    def forward(self, x):
        ...
        srb1_2_mem = srb1_2_spike = torch.zeros(batch_size, self.scale*4, 32, 32, device=self.device)
        srb2_1_mem = srb2_1_spike = torch.zeros(batch_size, self.scale, 32, 32, device=self.device)
        srb2_2_mem = srb2_2_spike = torch.zeros(batch_size, self.scale*4, 32, 32, device=self.device)
        srb3_1_mem = srb3_1_spike = torch.zeros(batch_size, self.scale*2, 16, 16, device=self.device)
        srb3_2_mem = srb3_2_spike = torch.zeros(batch_size, self.scale*2*4, 16, 16, device=self.device)
        srb4_1_mem = srb4_1_spike = torch.zeros(batch_size, self.scale*2, 16, 16, device=self.device)
        srb4_2_mem = srb4_2_spike = torch.zeros(batch_size, self.scale*2*4, 16, 16, device=self.device) 
        srb5_1_mem = srb5_1_spike = torch.zeros(batch_size, self.scale*4, 8, 8, device=self.device)
        srb5_2_mem = srb5_2_spike = torch.zeros(batch_size, self.scale*4*4, 8, 8, device=self.device) 
        srb6_1_mem = srb6_1_spike = torch.zeros(batch_size, self.scale*4, 8, 8, device=self.device)
        srb6_2_mem = srb6_2_spike = torch.zeros(batch_size, self.scale*4*4, 8, 8, device=self.device) 
        srb7_1_mem = srb7_1_spike = torch.zeros(batch_size, self.scale*8, 4, 4, device=self.device)
        srb7_2_mem = srb7_2_spike = torch.zeros(batch_size, self.scale*8*4, 4, 4, device=self.device) 
        srb8_1_mem = srb8_1_spike = torch.zeros(batch_size, self.scale*8, 4, 4, device=self.device)
        srb8_2_mem = srb8_2_spike = torch.zeros(batch_size, self.scale*8*4, 4, 4, device=self.device)        
